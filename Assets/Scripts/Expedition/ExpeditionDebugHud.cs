...
public class ExpeditionDebugHud : MonoBehaviour
{
    ...
    [SerializeField] private Vector2 size = new Vector2(420f, 274f);
    ...
    private void OnGUI()
    {
        ...
        GUI.Label(new Rect(position.x + 12f, position.y + 196f, size.x - 24f, 24f), $"Run Loot: {manager.GoldCollected} Gold  {manager.RelicsCollected} Relics");
        GUI.Label(new Rect(position.x + 12f, position.y + 220f, size.x - 24f, 24f), $"Carried: {pouch.Gold} Gold  {pouch.Relics} Relics");
        GUI.Label(new Rect(position.x + 12f, position.y + 244f, size.x - 24f, 24f), $"Extract: {manager.ExtractionStatus}");
    }
